...
export default function AdminFamiliaAcolhedoraPage() {
  ...
  const load = async () => {
    try {
      setItems(await adminFetch<App[]>("/admin/foster-families"));
    } catch (e) {
  ...
  const setStatus = async (id: string, status: string) => {
    try {
      await adminFetch(`/admin/foster-families/${id}/status`, {
        method: "PATCH",
  ...
  return (
    <div>
      <AdminPageHeader
        ...
        description="Inscrições de famílias interessadas em acolher temporariamente."
      />
      {error ? <ErrorBox text={error} /> : null}
      {ok ? <SuccessBox text={ok} /> : null}
      {!items.length ? (
        <EmptyState text="Nenhuma inscrição recebida." />
      ) : (
